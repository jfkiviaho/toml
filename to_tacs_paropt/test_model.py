...
if __name__ == '__main__':
    ...
    # Root communicator
    comm = MPI.COMM_WORLD
    # Test the model's orientation
    # Load in the mesh
    struct_mesh = TACS.MeshLoader(comm)
    struct_mesh.scanBDFFile(fname)
    # Create the isotropic material class
    props = constitutive.MaterialProperties(E=70e3, rho=1.0, nu=0.3)
    # Get the number of elements
    ...
    for i in range(num_elems):
        # Set the design variable index
        design_variable_index = i
        stiff = constitutive.SimpPSConstitutive(props, t=1.0, tNum=i,q=3.0,
                                                tlb=1e-3,tub=1.0)
        # Get the element model class
        model = elements.LinearElasticity2D(stiff)
        # Get the basis functions
        ...
        struct_mesh.setElement(i, element)
    # Create tacs assembler object from mesh loader
    assembler = struct_mesh.createTACS(2)
    funcs = [functions.StructuralMass(assembler)]
    # Evaluate the mass
    fval = assembler.evalFunctions(funcs)
    print(fval)
